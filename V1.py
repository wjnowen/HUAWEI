...
# 处理数据量较大的user_app_usage.csv，结合app_info.csv统计得到appuseProcessed.csv作为特征
def f(x):
    s = x.value_counts()
...
def processUserAppUsage():
    ...
    resTable.to_csv("model.csv", index=0)
...
def mergeAppData(baseTable):
    ...
    appusedTable = pd.read_csv("model.csv")
    ...
    duration_mean = resTable['duration_mean'].mean()
    duration_std = resTable['duration_mean'].std()
    time_mean = resTable['times_mean'].mean()
    time_std = resTable['times_mean'].std()
    usage_mean = resTable['usage_most_used_category'].mean()
    usage_std = resTable['usage_most_used_category'].std()
    resTable["duration_mean"] = (resTable["duration_mean"] - duration_mean) / duration_std
    resTable["times_mean"] = (resTable["times_mean"] - time_mean) / time_std
    resTable['usage_most_used_category'] = (resTable['usage_most_used_category'] - usage_mean) / usage_std
...
for index, (train_index, vali_index) in enumerate(skf.split(X, y)):
    print(index)
    x_train, y_train, x_vali, y_vali = np.array(X)[train_index], np.array(y)[train_index], np.array(X)[vali_index], \
                                       np.array(y)[vali_index]
...
df.to_csv(result_path + 'submission.csv', index=False)
